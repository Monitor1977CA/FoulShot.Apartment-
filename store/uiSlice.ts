...
interface UiState {
  ...
  /** A map from location ID to a boolean indicating if its info panel is expanded. */
  locationPanelExpandedState: { [locationId: string]: boolean; };
}
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    setActiveCard(state, action: PayloadAction<{ id: string; type: CardType, collectionType?: string; title?: string }>) {
      state.history.push({ 
          view: state.activeView, 
          cardId: state.activeCardId, 
          cardType: state.activeCardType,
    ...
    /**
     * Sets the expanded state for a specific location's info panel.
     */
    setLocationPanelExpanded(state, action: PayloadAction<{ locationId: string; isExpanded: boolean }>) {
      const { locationId, isExpanded } = action.payload;
      state.locationPanelExpandedState[locationId] = isExpanded;
    },
  },
});

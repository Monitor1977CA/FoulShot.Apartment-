...
const TimelineEventCard: React.FC<TimelineEventCardProps> = React.memo(({ evidence, isExpanded }) => {
    ...
    // Rarity display has been removed per user request.
    const borderClasses = `border-2 ${isExpanded ? 'border-brand-primary' : 'border-brand-border hover:border-brand-primary'}`;
    const borderColorStyle = {};
    
    const shadowClass = isExpanded ? 'shadow-brand-primary/20' : '';

    // Graceful fallback for the description text.
    // This is the key to ensuring backwards compatibility.
    const descriptionToShow = fullObjectData?.metadata?.timelineSummary || fullObjectData?.description;
        ...
        <div className={`bg-brand-surface rounded-lg shadow-lg w-72 flex-shrink-0 animate-snap-in transition-all duration-300 ${shadowClass} ${borderClasses}`} style={borderColorStyle}>
            ...
            <div className="p-3">
                <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    {/* Rarity badge display has been removed per user request. */}
                </div>
                ...
                {isExpanded && (
                    <div className="animate-fade-in mt-3 space-y-3">
                        <p className="text-sm text-brand-text mb-2 line-clamp-5">{descriptionToShow}</p>

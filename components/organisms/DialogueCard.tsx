...
import { createEvidenceFromTestimony } from '../../store/storySlice';
...
const DialogueCard: React.FC<DialogueCardProps> = ({ character }) => {
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    if (dialogueData.mode === 'interrogation' && activeLoi) {
    ...
    if (parsed) {
        if (dialogueData.mode === 'interrogation') {
            ...
            setActiveFeedback({ text: parsed.adaFeedback, progressChange: 0, source: 'question' });
  ...
  }, [isAiResponding, dialogueData, sendMessage, activeLoi, initialQuestions, character.id]);
  ...
  const handleCreateEvidence = useCallback((chunk: DialogueChunkData) => {
    ...
    setActiveFeedback({ text: 'Evidence logged. This is a relevant statement.', progressChange: 0, source: 'evidence' });
    ...
    return (
      <div className="flex-1 flex flex-col overflow-hidden">
        {dialogueData.mode === 'interrogation' && activeLoi ? (
          ...
          />
        ) : dialogueData.mode === 'interview' ? (
           <div className="p-4 bg-brand-surface border-b-2 border-brand-border flex-shrink-0">
            <BodyCamView
              characterId={character.id}
              witnessName={character.name}
              prompts={dialogueData.slideshowPrompts}
              currentImageIndex={currentImageIndex}
            />
          </div>
        ) : null}
